...
> **Workflow:** After every script edit, always copy to DCS runtime immediately — no need to ask:
> `Copy-Item -Path "C:\Users\johnk\Git\dcs\scripts\*" -Destination "C:\Users\johnk\Saved Games\DCS\Scripts\a2g_dynamic_syria\" -Recurse -Force`
...
- **Weapon objects can be `event.initiator` in `S_EVENT_KILL`** — not just in `S_EVENT_DEAD`. Any Unit-method helper (e.g. `getPlayerName`, `isEnemyUnit`) must guard with `type(unit.getCategory) ~= "function"` before calling `isExist()` or anything else; weapon objects lack these methods and calling them throws `attempt to call method 'isExist' (a nil value)`.
- **`pcall`-wrap the entire `onEvent` body** — a single unguarded error silently kills the event handler for the rest of the session. Wrap in `pcall` and log via `env.info`; scoring degrades gracefully instead of going silent.
- **AI kills must suppress the HIT→DEAD fallback** — `S_EVENT_KILL` fires for AI kills too, but the old code returned early on `getPlayerName(AI) == nil` without setting `killCredited`. Then `S_EVENT_DEAD` found a stale `lastHitBy` from a human who had previously hit the same unit and incorrectly credited them. Fix: always set `killCredited[unitName] = true` and clear `lastHitBy[unitName]` in `S_EVENT_KILL` *before* checking whether the killer is a player.
- **`weaponToPlayer` cache must be pruned on weapon death** — DCS can reuse userdata pointers for weapon objects after they die. A cached human player entry for a dead weapon could later match a new AI weapon in the HIT handler's last-resort cache lookup. Fix: handle `Object.Category.WEAPON` in `S_EVENT_DEAD` and nil out the entry.

### Cost Config Research (cost_config.lua)
- All SAM system and ground vehicle kill values updated to 2026 inflation-adjusted replacement costs. Sources: ForecastInternational archives, Ukraine war damage assessments (Oryx/defense-ua), export contract prices divided by vehicle count, CPI adjustment from documented sale years.
- Most reliable single-system value: **Tunguska SA-19 = $15.1M** — explicit ForecastInternational cite.
- Confidence varies: S-300 and Buk values are medium-confidence (battery contracts ÷ vehicle count); SA-2/SA-3 are low-confidence (estimated from India and other deals with limited per-vehicle granularity).
- Logistics trucks ($0.08–0.15M real cost) are 3–5× lower than the previous $0.3–0.5M values. Updated to real figures. If convoy kills feel undervalued for gameplay, raise these deliberately — do not inflate SAM/armor values to compensate.
